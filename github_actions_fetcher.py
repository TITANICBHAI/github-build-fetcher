...
h1{font-size:clamp(38px,6vw,64px);line-height:.98;letter-spacing:-.06em;margin:13px 0 15px}.lead{color:#aebbd2;font-size:17px;line-height:1.5;max-width:650px;margin-bottom:27px}
 ...
 .legal-gate{position:fixed;inset:0;z-index:20;display:flex;align-items:center;justify-content:center;padding:22px;background:#070b16dd;backdrop-filter:blur(8px)}.legal-gate[hidden]{display:none}.legal-box{max-width:650px;background:#16243c;border:1px solid #6681ad;border-radius:18px;padding:26px;box-shadow:0 24px 90px #000b;color:#d9e4f5}.legal-box h2{margin:0 0 10px;color:#fff;font-size:24px}.legal-box p{font-size:13px;line-height:1.55;color:#b9c7dc}.legal-box ul{font-size:12px;line-height:1.55;color:#b9c7dc;padding-left:19px}.legal-box a{color:#8de4d8}.legal-check{display:flex;gap:10px;align-items:flex-start;margin:18px 0;color:#f0f5fc;font-size:13px;line-height:1.4}.legal-check input{width:auto;margin-top:2px;accent-color:#73d7c9}
...
@media(max-width:740px){.grid{grid-template-columns:1fr}.wrap{padding-top:30px}.facts{grid-template-columns:1fr}}
 </style></head><body><div id="legalGate" class="legal-gate"><section class="legal-box" role="dialog" aria-modal="true" aria-labelledby="legalTitle"><h2 id="legalTitle">Before you use BuildFetchr</h2><p>BuildFetchr is independent third-party software for working with GitHub-hosted data. It is not affiliated with, endorsed, sponsored, or maintained by GitHub.</p><p>Use it at your own risk. We do not promise uninterrupted service, security, accuracy, availability, or preservation of data. You are responsible for your account, tokens, backups, permissions, downloaded files, and any actions you take through the app.</p><ul><li>Do not enter credentials you are not authorized to use.</li><li>Do not rely on BuildFetchr for backups, compliance, production recovery, or other critical operations.</li><li>To the maximum extent permitted by law, BuildFetchr and its contributors are not liable for loss, corruption, unauthorized access, disclosure, interruption, service failure, malware, device damage, financial loss, or other direct, indirect, incidental, special, consequential, or regulatory harm arising from use of the software.</li></ul><p>Read the <a href="https://github.com/TITANICBHAI/github-build-fetcher/blob/main/docs/terms.html" target="_blank" rel="noopener">Terms</a> and <a href="https://github.com/TITANICBHAI/github-build-fetcher/blob/main/docs/privacy.html" target="_blank" rel="noopener">Privacy Policy</a>. Nothing here excludes rights or remedies that cannot legally be excluded.</p><label class="legal-check"><input id="acceptLegal" type="checkbox"> <span>I understand and accept these terms and the responsibility limitations before using BuildFetchr.</span></label><button id="acceptLegalButton" type="button" disabled>Accept and continue</button></section></div><main class="wrap"><div class="eyebrow">BuildFetchr · independent GitHub community utility</div><h1>Bring a build home.</h1>
...
<script>
 ...
 const legalAccepted=localStorage.getItem('buildfetchr-terms-accepted')==='1';$('legalGate').hidden=legalAccepted;
 $('acceptLegal').onchange=()=>{$('acceptLegalButton').disabled=!$('acceptLegal').checked};
 $('acceptLegalButton').onclick=()=>{if(!$('acceptLegal').checked)return;localStorage.setItem('buildfetchr-terms-accepted','1');$('legalGate').hidden=true};
